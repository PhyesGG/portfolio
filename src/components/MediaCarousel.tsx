...
function CarouselControl({ isLeft = false, handleNext, handlePrevious }: { isLeft?: boolean, handleNext: () => void, handlePrevious: () => void }) {
    return (
        <button
            ...
            className={`absolute z-10 flex h-8 w-8 items-center justify-center self-center rounded-full bg-noir text-sm text-ivory opacity-90 md:text-2xl ${isLeft ? 'md:left-8 left-6' : 'md:right-8 right-6'}`}
        >
